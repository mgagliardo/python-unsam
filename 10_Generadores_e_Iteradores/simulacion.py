...
class Animal(object):
    ...
    def pasar_un_ciclo(self):
        ...
        self.edad += 1
        if self.reproducciones_pendientes > 0 and self.edad >= 2: # Solo puede reproducir si >= 2 años
...
class Mundo(object):
    ...
    def etapa_reproduccion(self):
        ...
        for p in self.tablero.posiciones_ocupadas():
            animal = self.tablero.posicion(p)
            animales_cercanos = self.tablero.posiciones_vecinas_con_ocupantes(p)
            # Solo se ejecuta si puede reproducirse.
            if animal.puede_reproducir():
                # Se eligen los animales cercanos y de la misma especie
                if animal.es_antilope:
                    animales_cercanos = [vecino[1] for vecino in animales_cercanos if vecino[1].es_antilope() and vecino[1].puede_reproducir()]
                elif animal.leon:
                    animales_cercanos = [vecino[1] for vecino in animales_cercanos if vecino[1].es_leon() and vecino[1].puede_reproducir()]
                posiciones_libres = self.tablero.posiciones_vecinas_libre(p)
                cria = animal.reproducirse(animales_cercanos, posiciones_libres)
                if cria:
                    # Por ahora solo antilopes y leones. En caso de extender los animales deberia reconsiderarse este if
                    tipo_cria = Antilope() if animal.es_antilope else Leon()
                    self.tablero.ubicar_en_posicion_vacia(tipo_cria)
